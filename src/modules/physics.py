...
def WD_radius(m: float) -> float:
    ...
    r = (
        0.0114
        * np.sqrt((m / 1.44) ** (-2 / 3) - (m / 1.44) ** (2 / 3))
        * (1 + 3.5 * (m / 0.00057) ** (-2 / 3) + 0.00057 / m) ** (-2 / 3)
    )
    return r
...
def Kepler(m1: float, m2: float) -> float:
    ...
    nu = np.sqrt(
        ...
        * cst.M_sun
        * (m1 + m2)
        / (4 * np.pi**2 * (a_min(m1, m2) * cst.R_sun) ** 3)
    )
    return nu.value
...
def Period(a: float, m1: float, m2: float) -> np.array:
    ...
    """
    return (
        (a * cst.R_sun) ** 3 * 4 * np.pi**2 / (cst.G * cst.M_sun * (m1 + m2))
    ) ** (1 / 2)
